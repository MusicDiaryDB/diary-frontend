...
export const addNewSongFromGeniusSearch = async (geniusSongId: number) => {
  ...
  try {
    ...
    try {
      ...
      if (!getAlbumResponse) {
        getAlbumResponse = await addAlbum(
          geniusSongDetails.albumName,
          getArtistResponse?.artistId,
        );
        isAlbumMutated = true;
      }
      ...
      try {
        ...
        if (!getSongResponse) {
          getSongResponse = await addSong(
            geniusSongDetails.releaseDate,
            geniusSongDetails.title,
            getAlbumResponse.albumId,
          );
          isSongMutated = true;
        }
      } catch (err) {
...
export const getSongByID = async function (songId: number) {
  try {
    const resp = await diaryClient.get(`/song/${songId}`);
    const res = resp.data;
    return res;
  } catch (error) {
